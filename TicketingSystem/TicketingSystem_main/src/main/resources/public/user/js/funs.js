...
//				 			})
    ...
    this.selectAll = function () {
        var obj = {};
        $(".layui-form input , .layui-form select ").each(function () {	//each作循环
            var v = $(this).val();
            var n = $(this).attr("name");
            if (n) obj[n] = v;	//向obj中添加属性
        })
...
function LayForm() {
    ...
    this.init = function () {
        layui.use(["layer", "jquery", "form", "laydate"], function () {
            ...
            if (self.loaded) self.loaded();	//将该事件加载完成，加载后才能实现相关功能
        });
    }
    this.render = function () {
        form.render();
    }
    this.setval = function (json) {	//将接收到的记录直接复制到myform内
        form.val("myform", json);
    }
}


function ajax_array(lay, el, url, def) {
    el.empty();
    $.get(url, {}, function (json) {
